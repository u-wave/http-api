...
export async function replaceBooth(uw, moderatorID, id) {
  ...
  if (!waitlist.length) throw new HTTPError(404, 'waitlist is empty');
...
export async function favorite(uw, id, playlistID, historyID) {
  ...
  if (!historyEntry) {
    throw new HTTPError(404, `history entry with ID ${historyID} not found`);
  }
  if (`${historyEntry.user}` === id) {
    throw new HTTPError(403, 'you can\'t grab your own song');
  }
  ...
  if (`${playlist.author}` !== id) {
    throw new HTTPError(403, 'you are not allowed to edit playlists of other users');
  }
